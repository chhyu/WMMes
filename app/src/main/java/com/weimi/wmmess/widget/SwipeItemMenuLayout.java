 ...
 * 【Item侧滑删除菜单】
 */
public class SwipeItemMenuLayout extends ViewGroup {
    ...
    //存储的是当前正在展开的View
    private static SwipeItemMenuLayout mViewCache;
    ...
    public SwipeItemMenuLayout(Context context) {
        this(context, null);
    }

    public SwipeItemMenuLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SwipeItemMenuLayout(Context context, AttributeSet attrs, int defStyleAttr) {
     ...
     * @param ios
     */
    public SwipeItemMenuLayout setIos(boolean ios) {
        isIos = ios;
        return this;
    }
     ...
     * @return
     */
    public SwipeItemMenuLayout setLeftSwipe(boolean leftSwipe) {
     ...
     * @return
     */
    public static SwipeItemMenuLayout getViewCache() {
        return mViewCache;
    }
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        //左滑右滑的开关,默认左滑打开菜单
        isLeftSwipe = true;
        TypedArray ta = context.getTheme().obtainStyledAttributes(attrs, R.styleable.SwipeItemMenuLayout, defStyleAttr, 0);
        int count = ta.getIndexCount();
        for (int i = 0; i < count; i++) {
            int attr = ta.getIndex(i);
            //如果引用成AndroidLib 资源都不是常量，无法使用switch case
            if (attr == R.styleable.SwipeItemMenuLayout_swipeEnable) {
                isSwipeEnable = ta.getBoolean(attr, true);
            } else if (attr == R.styleable.SwipeItemMenuLayout_ios) {
                isIos = ta.getBoolean(attr, true);
            } else if (attr == R.styleable.SwipeItemMenuLayout_leftSwipe) {
        ...
        if (isSwipeEnable) {
            ...
            switch (ev.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE:
                    ...
                    if (Math.abs(gap) > 10 && Math.abs(gap) > Math.abs(gapY)) {//2016 09 29 修改此处，使屏蔽父布局滑动更加灵敏，
                        getParent().requestDisallowInterceptTouchEvent(true);
                        isSliding = true;
                    }
...
/*                    if (!mScroller.isFinished()) {
    ...
    public void smoothExpand() {
        ...
        invalidate();*/
        //展开就加入ViewCache：
        mViewCache = SwipeItemMenuLayout.this;
